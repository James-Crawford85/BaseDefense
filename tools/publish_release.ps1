...
if ([string]::IsNullOrWhiteSpace($Notes)) {
    $Notes = "Base Defense $tag. Download, extract, keep all files together, run BaseDefense.exe with Steam running. In-game: the updater will offer this automatically to anyone on an older build."
}
gh release create $tag $zip --repo James-Crawford85/BaseDefense --title "Base Defense $tag" --notes $Notes
Write-Host "== Done: https://github.com/James-Crawford85/BaseDefense/releases/tag/$tag ==" -ForegroundColor Green
